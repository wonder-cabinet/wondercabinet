...
//   - "Neue Haas Grotesk Display Pro 55 Roman" -- all English text.
//   - "Nabi" -- all Arabic text (cover week-ordinal label, and every
//     grid day header/time/title/description, both digital and print).
//     Confirmed directly against the real Figma layer (2026-09-21):
//     an earlier version of this file guessed Alyamama for the grid's
//     Arabic body text -- that was wrong, it's Nabi throughout.
//   - "Alyamama" -- loaded but currently unused; kept in case a future
//     design brings it back in for some element.
//
...
// Print page (A3, much taller canvas): a proper row-major 3-column grid
// that wraps to additional rows instead of stacking overflow in column 3
// -- verified against the real print reference (A3 PRINT.svg's 6-event
// sample fills 2 full rows of 3 columns, row-major left-to-right).
function groupByDayGrid(events) {
  const days = groupDays(events);
  const rows = [];
  for (let i = 0; i < days.length; i += 3) rows.push(days.slice(i, i + 3));
  return rows;
}
...
<g fill="${theme.fg}" text-anchor="start" class="ar-title">
  <text x="${margin}" y="${H - margin - 82}" font-size="40" direction="rtl">${esc(`الأسبوع ${ordinal}`)}</text>
  <text x="${margin}" y="${H - margin - 28}" font-size="40" direction="rtl">${esc(formatRangeAr(weekly.weekStart, weekly.weekEnd))}</text>
</g>
...
function scriptFont(text, columnFont) {
  return ARABIC_RE.test(text) ? columnFont : "Haas";
}
...
function dayColumn(day, evs, ar, colFontScale, fg, maxWidth) {
  ...
  evs.forEach((ev, i) => {
    ...
    if (desc) {
      ...
      const descFont = scriptFont(desc, fontFam);
  ...
  const textEls = lines
    .map((l) => {
      ...
      const x = ar && lineIsArabic ? -measureWidth(l.text, l.font, l.size) : 0;
...
export function buildGridSVG(weekly, dir, theme, opts = {}) {
  ...
  const arHeaderX1 = W - margin - measureWidth(arHeaderLine1, "Nabi", 53);
  const arHeaderX2 = W - margin - measureWidth(arHeaderLine2, "Nabi", 36);
  const headerSVG = hideHeader
    ? ""
    : ar
    ? `<g text-anchor="start" fill="${theme.fg}" font-family="Nabi" direction="rtl">
        <text x="${arHeaderX1}" y="${margin + 50}" font-size="53" font-weight="700">${esc(arHeaderLine1)}</text>
        <text x="${arHeaderX2}" y="${margin + 95}" font-size="36">${esc(arHeaderLine2)}</text>
      </g>`
  ...
  cols.forEach((col, i) => {
    ...
    col.forEach(({ day, evs }) => {
      ...
      voffset += block.height + Math.round(70 * colFontScale); // gap before next stacked day-block
    });
  });
...
function printHeaderInner(weekly, theme) {
  const weekNum = weekOfMonth(weekly.weekStart);
  const ordinal = ORDINAL_AR[weekNum - 1] || arDigits(weekNum);
  ...
  // Same right-alignment fix as dayColumn's textEls / buildGridSVG's header
  // (see dayColumn's comment for the full explanation): text-anchor="start"
  // + direction="rtl" doesn't right-anchor RTL text in resvg, so these two
  // lines are measured and positioned to end just before the English block
  // (which starts at x=347) instead of relying on a fixed x=248 that
  // assumed the anchor would right-align them there. Found 2026-09-21: the
  // fixed x=248 let "الأسبوع الرابع" run straight through "WEEK NO.4" in
  // the real generated PDF.
  const printArLine1 = `الأسبوع ${ordinal}`;
  const printArLine2 = formatRangeAr(weekly.weekStart, weekly.weekEnd);
  const printArRightEdge = 330;
  const printArX1 = printArRightEdge - measureWidth(printArLine1, "Nabi", 34);
  const printArX2 = printArRightEdge - measureWidth(printArLine2, "Nabi", 30);

  return `<g transform="translate(30,25) scale(${calScale})" color="${theme.fg}">${CALLIGRAPHY_COVER.inner}</g>
<g fill="${theme.fg}" text-anchor="start" font-family="Nabi" direction="rtl">
  <text x="${printArX1}" y="280" font-size="34" font-weight="700">${esc(printArLine1)}</text>
  <text x="${printArX2}" y="320" font-size="30">${esc(printArLine2)}</text>
</g>
<g fill="${theme.fg}" text-anchor="start" font-family="Haas" font-weight="700" text-transform="uppercase">
  <text x="347" y="295" font-size="32">WEEK NO.${weekNum}</text>
  <text x="347" y="330" font-size="32">${esc(formatRangeEnHeader(weekly.weekStart, weekly.weekEnd))}</text>
</g>
<g fill="${theme.fg}" text-anchor="end" font-family="Haas" font-weight="700" font-size="24">
  <text x="812" y="220">THIS</text>
  <text x="812" y="250">WEEK</text>
  <text x="812" y="280">AT THE</text>
  <text x="812" y="310">WONDER</text>
  <text x="812" y="340">CABINET</text>
</g>`;
}
...
function renderPrintGrid(weekly, ar, theme, { W, margin, colGap, colFontScale, startY, mirror = ar }) {
  ...
  // laid-out blocks.
  const rows = groupByDayGrid(weekly.events);
  const colW = (W - margin * 2 - colGap * 2) / 3;
  let rowTop = startY;
  let svg = "";
  rows.forEach((row) => {
    let rowHeight = 0;
    row.forEach(({ day, evs }, ci) => {
      const cx = mirror ? W - margin - colW * (ci + 1) - colGap * ci : margin + colW * ci + colGap * ci;
      const anchorX = ar ? cx + colW : cx;
      const block = dayColumn(day, evs, ar, colFontScale, theme.fg, colW);
      svg += `<g transform="translate(${anchorX},${rowTop})">${block.svg}</g>`;
      rowHeight = Math.max(rowHeight, block.height);
    });
    rowTop += rowHeight + Math.round(70 * colFontScale);
  });
  return { svg, bottom: rowTop };
}
...
export function buildPrintPageSVG(weekly, dir, theme, opts = {}) {
  ...
  const headerSVG = showHeader ? `<g>${printHeaderInner(weekly, theme)}</g>` : "";
...
export function buildPrintCombinedSVG(weekly, theme) {
  ...
  const headerSVG = `<g>${printHeaderInner(weekly, theme)}</g>`;
