...
//   - "Nabi" -- Arabic display/headline text (the calligraphy tagline,
//     week-ordinal headline).
//   - "Alyamama" -- Arabic body text (day names, times, titles,
//     descriptions).
//
...
function innerOf(svgPath) {
  const raw = readFileSync(path.join(__dirname, svgPath), "utf8");
...
const CALLIGRAPHY_COVER = innerOf("calligraphy.svg"); // 748.7 x 280.5
...
// a Latin font (Haas), not the column's nominal Arabic body font
// (Alyamama), regardless of which slide it's on -- confirmed necessary by
// testing against real production data, most of which has no Arabic title
// yet. This picks the right font per PIECE OF TEXT, not per column.
...
function dayColumn(day, evs, ar, colFontScale, fg, maxWidth) {
  ...
  const fontFam = ar ? "Alyamama" : "Haas";
  let y = 0;
  const lines = [];
  ...
  const textEls = lines
    .map((l) => {
      ...
      return `<text x="0" y="${l.y}" font-size="${l.size}" font-weight="${l.weight}" opacity="${l.opacity}" font-family="${l.font}" direction="${dir}" text-anchor="${anchor}">${esc(l.text)}${supTspan}</text>`;
    })
    .join("\n");
...
export function buildGridSVG(weekly, dir, theme, opts = {}) {
  ...
  const headerSVG = hideHeader
    ...
    ? `<g text-anchor="start" fill="${theme.fg}" font-family="Nabi" direction="rtl">
        <text x="${W - margin}" y="${margin + 50}" font-size="53" font-weight="700">${esc(`الأسبوع ${ordinal}`)}</text>
        <text x="${W - margin}" y="${margin + 95}" font-size="36">${esc(formatRangeAr(weekly.weekStart, weekly.weekEnd))}</text>
      </g>`
...
<g fill="${theme.fg}" text-anchor="start" font-family="Nabi" direction="rtl">
  <text x="248" y="280" font-size="34" font-weight="700">${esc(`الأسبوع ${ordinal}`)}</text>
  <text x="248" y="320" font-size="30">${esc(formatRangeAr(weekly.weekStart, weekly.weekEnd))}</text>
</g>
...
export function buildPrintCombinedSVG(weekly, theme) {
  ...
  const sectionGap = 50;
  // mirror:false -- keep the AR section's columns in the exact same
  // physical slots as the EN section above (one continuous grid running
  // top to bottom), instead of mirroring column order per RTL convention.
  const ar = renderPrintGrid(weekly, true, theme, {
    W, margin, colGap, colFontScale, startY: en.bottom + sectionGap, mirror: false,
  });
