...
function printHeaderInner(weekly, theme, margin = 52, W = 842, colGap = 30) {
  const weekNum = weekOfMonth(weekly.weekStart);
  const ordinal = ORDINAL_AR[weekNum - 1] || arDigits(weekNum);
  ...
  // Ibrahim marked up a print PDF 2026-09-23 with two arrows: move the
  // "WEEK NO.X"/date block right to line up with the grid's 2nd column
  // (THU) below it, and align the tagline's widest line ("WONDER") so
  // its own left edge (the "W") lines up with the grid's 3rd column's
  // (SAT) left edge (the "S") -- i.e. align the header's text blocks to
  // the same 3-column grid the day columns use just below (see
  // renderPrintGrid's identical colW math), rather than treating the
  // header as independent of the grid geometry. First tried right-
  // anchoring the whole tagline block at col2X directly, which pointed
  // the RIGHT edge of every line (not "WONDER"'s left edge) at col2X and
  // collided with the WEEK NO. block -- Ibrahim clarified the ask is
  // specifically "WONDER"'s left edge against SAT's left edge, not a
  // block-level right-anchor.
  const colW = (W - margin * 2 - colGap * 2) / 3;
  const col1X = margin + colW + colGap; // 2nd column's (THU) left edge
  const col2X = margin + colW * 2 + colGap * 2; // 3rd column's (SAT) left edge

  // Still guards against the "WEEK NO.X" block colliding with the Arabic
  // ordinal block to its left (see the pre-2026-09-23 version of this
  // function): if a wider ordinal word ever pushes past the col1 target,
  // fall back to the old measured-width-based offset instead of
  // overlapping it. In the common case (like this week's "الأسبوع
  // الرابع") col1X already clears it with room to spare.
  ...
  const enX = Math.max(col1X, margin + arBlockWidth + 20);

  // Tagline font-size matched to the week-number/date block (32, was 24)
  // per Ibrahim's ask 2026-09-23. Line spacing now ALSO matches that
  // block's own two-line spacing (35, was a looser 40 chosen when the
  // tagline's line height wasn't tied to anything else) -- shared via
  // weekLineY/dateLineY below rather than a separate hardcoded number, so
  // the two blocks can't drift apart again if either one's y changes.
  const taglineSize = 32;
  const weekLineY = 295; // the "WEEK NO.X" line's y, below
  const dateLineY = 330; // the "SEP 21 - 27" line's y, below -- shared so the two can be bottom-aligned
  const taglineSpacing = dateLineY - weekLineY;
  const tagline = ["THIS", "WEEK", "AT THE", "WONDER", "CABINET"];
  // Bottom-aligned with the date line per Ibrahim's follow-up ask: the
  // tagline's LAST line ("CABINET") sits on the same row as "SEP 21-27",
  // not a fixed top -- so taglineTop is derived backwards from where the
  // last line needs to land, rather than the block just starting at a
  // fixed y and however it happens to end.
  const taglineTop = dateLineY - (tagline.length - 1) * taglineSpacing;
  // The block stays right-anchored (ragged-left, flush-right, as before)
  // so all 5 lines keep sharing one x -- but that shared x is now derived
  // from "WONDER" specifically (the widest line) so ITS left edge, not
  // the block's right edge, lands on col2X.
  const taglineAnchorX = col2X + measureWidth("WONDER", "Haas", taglineSize);
  const taglineSVG = tagline
    .map((t, i) => `<text x="${taglineAnchorX}" y="${taglineTop + i * taglineSpacing}">${t}</text>`)
    .join("\n  ");
...
<g fill="${theme.fg}" text-anchor="start" font-family="Haas" font-weight="700" text-transform="uppercase">
  <text x="${enX}" y="${weekLineY}" font-size="32">WEEK NO.${weekNum}</text>
  <text x="${enX}" y="${dateLineY}" font-size="32">${esc(formatRangeEnHeader(weekly.weekStart, weekly.weekEnd))}</text>
</g>
...
export function buildPrintPageSVG(weekly, dir, theme, opts = {}) {
  ...
  const headerSVG = showHeader ? `<g>${printHeaderInner(weekly, theme, margin, W, colGap)}</g>` : "";
...
export function buildPrintCombinedSVG(weekly, theme) {
  ...
  const headerSVG = `<g>${printHeaderInner(weekly, theme, margin, W, colGap)}</g>`;
