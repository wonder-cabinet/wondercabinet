...
// The digest only has room for one short line under the title (per the
// approved WK03 design). Use just the first line, capped, rather than
// dumping a whole paragraph into a grid cell -- kept as a safety net now
// that the source is `subtitle` (a short localeString, not a long blurb),
// in case someone pastes something long into it.
...
function adaptEvent(ev) {
  ...
  // Ibrahim asked 2026-09-23 to use the event's `subtitle` field (a short
  // one-line localeString, e.g. "Mats and props will be provided") for the
  // grid's description line instead of `shortDescription` (a longer
  // localeText blurb meant for elsewhere on the site, e.g. the event's own
  // page) -- subtitle is the field actually meant to be a short line, and
  // shortDescription's real content was often multiple paragraphs (the
  // Mahmoud Darwish event's shortDescription ran to 6+ lines before
  // briefDesc() truncated it to just its first line).
  const enDescRaw = ev.subtitle && ev.subtitle.en;
  const arDescRaw = ev.subtitle && ev.subtitle.ar;
