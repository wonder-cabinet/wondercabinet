...
export function buildCoverTree(weekly) {
  ...
  const weekNum = weekOfYear(weekStart);
  const margin = 77;
      ...
      children: [
        {
          type: "div",
          props: {
            style: { display: "flex", fontWeight: 700, fontSize: Math.round(48 * colFontScale), lineHeight: 1, fontFamily: "Amiri" },
            children: `${DWAR[d.getDay()]} ${arNum(d.getDate())}`,
          },
        },
        ...dayEvents.flatMap((ev, i) => {
          const timeParts = [formatTimeEn(ev.startDateTime), formatTimeEn(ev.endDateTime)].filter(Boolean);
          ...
          return [
            {
              ...
              props: {
                ...
                children: [
                  timeLabel
                    ? { type: "div", props: { style: { display: "flex", fontSize: Math.round(26 * colFontScale), fontWeight: 700, fontFamily: "Amiri" }, children: timeLabel } }
                    : null,
...
function secondaryRowAr(ev) {
  ...
  return {
    ...
    props: {
      ...
      children: [
        timeLabel ? { type: "div", props: { style: { display: "flex", fontSize: 36, fontWeight: 700, fontFamily: "Amiri" }, children: timeLabel } } : null,
...
export function buildGridTree(weekly, dir) {
  const weekStart = weekly.weekStart || new Date().toISOString();
  const weekEnd = weekly.weekEnd || weekStart;
  const weekNum = weekOfYear(weekStart);
...
export async function renderWeeklyEnPng(weekly) {
  const { ImageResponse } = await import("@vercel/og");
  const res = new ImageResponse(buildGridTree(weekly, "ltr"), { width: W, height: H, fonts: WEEKLY_FONTS });
  return Buffer.from(await res.arrayBuffer());
}

export async function renderWeeklyArPng(weekly) {
  const { ImageResponse } = await import("@vercel/og");
  const res = new ImageResponse(buildGridTree(weekly, "rtl"), { width: W, height: H, fonts: WEEKLY_FONTS });
  return Buffer.from(await res.arrayBuffer());
}

// A3 portrait (297x420mm), the 3 panels stacked vertically, each kept at
// its exact 4:5 design proportions (uniformly scaled to fit, never
// stretched/cropped) and centered — since the panel background is the same
// yellow as the page, there's no visible seam or letterboxing bar.
export async function renderWeeklyDigestPdf(weekly) {
  const { PDFDocument, rgb } = await import("pdf-lib");
  const [coverPng, enPng, arPng] = await Promise.all([renderWeeklyCoverPng(weekly), renderWeeklyEnPng(weekly), renderWeeklyArPng(weekly)]);
  ...
  page.drawRectangle({ x: 0, y: 0, width: pageW, height: pageH, color: rgb(1, 1, 0) });

  const panelAspect = H / W; // 1.25
  const maxPanelH = pageH / 3;
  ...
  const images = await Promise.all([coverPng, enPng, arPng].map((buf) => pdf.embedPng(buf)));
