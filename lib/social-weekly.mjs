...
// Print-only header, pulled from the real "A3 PRINT" Figma frame (file
// WC-DESIGN, node 403:141 -- linked directly by Ibrahim), which is its own
// dedicated print design distinct from the digital cover/en/ar slides:
// deep blue (#1500b3) text, no fill. Frame is 842x1191pt -- i.e. exactly
// A3 at 72dpi/points, the same units renderWeeklyDigestPdf's page uses, so
// these coordinates need no rescaling there.
const PRINT_BLUE = "#1500b3";
const HEADER_W = 842;
const HEADER_H = 400;
// Figma's mockup week used the Arabic ordinal word ("الأسبوع الثاني") in
// this header rather than a numeral -- matched here since weekOfMonth is
// always 1-5.
...
// "AUG 7 – 14" (month first) -- matches the exact wording in the Figma A3
// PRINT header, distinct from formatRangeEn's "7 - 14 AUG" used elsewhere.
...
// --- Print-only header (Figma "A3 PRINT" frame, node 403:141) -----------
//
// NOTE: the frame's centerpiece is a hand-drawn Arabic calligraphy graphic
// ("هذا الأسبوع في مجلس العجب" -- "this week at Wonder Cabinet") -- a
// vector illustration, not text in any installable font. Figma's API quota
// was exhausted while building this (MCP tool-call limit hit mid-session),
// so the graphic itself couldn't be pulled down; this substitutes a plain
// Amiri-bold rendering of the same phrase in its place. Swap in the real
// asset once Figma access is available again (or Ibrahim exports it).
export function buildPrintHeaderTree(weekly) {
  ...
  const ordinal = ORDINAL_AR[weekNum - 1] || arNum(weekNum);
      ...
      children: [
        // Calligraphy stand-in.
        {
          ...
          props: {
            ...
            children: "هذا الأسبوع في مجلس العجب",
          },
        },
        // Arabic week ordinal + date range -- left side of the page.
        {
          type: "div",
          props: {
            style: {
            ...
            children: [
              bidiRow([{ text: "الأسبوع" }, { text: ordinal }], { gap: 12, fontWeight: 700, fontSize: 34 }),
              { type: "div", props: { style: { display: "flex", fontSize: 30 }, children: formatRangeAr(weekStart, weekEnd) } },
            ],
          },
        },
        // "WEEK NO.X / AUG 7-14" -- middle.
        {
          type: "div",
          props: {
            style: {
            ...
            children: [
              { type: "div", props: { style: { display: "flex" }, children: `Week No.${weekNum}` } },
              { type: "div", props: { style: { display: "flex" }, children: formatRangeEnHeader(weekStart, weekEnd) } },
            ],
          },
        },
        // "THIS WEEK AT THE WONDER CABINET" -- right side of the page.
        {
          type: "div",
          props: {
            style: {
            ...
            children: "This Week At The Wonder Cabinet",
          },
        },
      ],
    },
...
export async function renderPrintHeaderPng(weekly) {
  const { ImageResponse } = await import("@vercel/og");
  const res = new ImageResponse(buildPrintHeaderTree(weekly), { width: HEADER_W, height: HEADER_H, fonts: WEEKLY_FONTS });
...
export function buildGridTree(weekly, dir, opts = {}) {
  const { background = YELLOW, hideHeader = false } = opts;
  const weekStart = weekly.weekStart || new Date().toISOString();
  const weekEnd = weekly.weekEnd || weekStart;
  const weekNum = weekOfMonth(weekStart);
  ...
  const columnsRow = {
    type: "div",
    props: {
      style: { display: "flex", flexDirection: isAr ? "row-reverse" : "row", gap: 40, marginTop: hideHeader ? 10 : 90 },
  ...
  return {
    ...
    props: {
      ...
      children: [hideHeader ? null : header, columnsRow, secondary].filter(Boolean),
    },
  };
}
...
// A3 portrait (297x420mm = 842x1191pt, same units as HEADER_W/HEADER_H).
// Header from the real Figma "A3 PRINT" frame on top (see
// buildPrintHeaderTree), then the EN + AR grid panels stacked below --
// the star-tracker cover panel is dropped entirely (Ibrahim: "for the
// print version its without the header with the star moving"; each grid
// panel keeps its own text header/date-range, just no star graphic).
// No background fill anywhere -- blank paper, only the rendered text/line
// colors ("without a background color for print, only the text color").
// Grid panels are kept at their exact 4:5 design proportions (uniformly
// scaled to fit, never stretched/cropped) and centered in the space left
// under the header.
export async function renderWeeklyDigestPdf(weekly) {
  const { PDFDocument } = await import("pdf-lib");
  const [headerPng, enPng, arPng] = await Promise.all([
    renderPrintHeaderPng(weekly),
    renderWeeklyEnPng(weekly, { background: "transparent", hideHeader: true }),
    renderWeeklyArPng(weekly, { background: "transparent", hideHeader: true }),
  ]);
  ...
  // Header spans the full page width at its native aspect ratio.
  const headerH = pageW * (HEADER_H / HEADER_W);
  page.drawImage(await pdf.embedPng(headerPng), { x: 0, y: pageH - headerH, width: pageW, height: headerH });

  const remainingH = pageH - headerH;
  const panelAspect = H / W; // 1.25
  const maxPanelH = remainingH / 2;
